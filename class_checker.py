...
print(f"max: {group['startTimes'].apply(len).max()}")
print(f"min: {group['startTimes'].apply(len).min()}")
print(f"mean: {group['startTimes'].apply(len).mean()}")
print(f"median: {group['startTimes'].apply(len).median()}")
print(f"std: {group['startTimes'].apply(len).std()}")
print(f"var: {group['startTimes'].apply(len).var()}")
print(f"sum: {group['startTimes'].apply(len).sum()}")
print(f"count: {group['startTimes'].apply(len).count()}")
print(f"describe: {group['startTimes'].apply(len).describe()}")
...
def minimize_check_sessions(day_time_list):
    ...
    while i < len(times):
        ...
        while i < len(times) and times[i] <= times[start_idx] + timedelta(minutes=15):
            latest_idx = i
            i += 1
...
for date, check_times in zip(group["date"], group["check_times"]):
    for check_time in check_times:
        new_row = {
            ...
            "All Day Event": False,
            "Description": "Epiphan check session 15 minutes",
            "Location": "Media Center",
            "Private": True,
        }
        calendar_df.loc[len(calendar_df)] = new_row

calendar_df.to_csv("epiphan_check_sessions.csv", index=False)
